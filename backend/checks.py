...
run_checks(alignment, idx_en, idx_zh) -> list of finding dicts WITHOUT severity/status:
...
def extract_currency(text: str, lang: str) -> list[Cur]:
    table = _EN_CUR if lang == "en" else _ZH_CUR
    out: list[Cur] = []
...
def _fmt_value(v: float, like: Num | None, cur: str | None) -> str:
    ...
    prefix = ""
    if cur:
        prefix = {"HKD": "HK$", "USD": "US$", "CNY": "RMB", "CAD": "C$", "EUR": "€", "GBP": "£"}.get(cur, cur)
...
_CUR_CODE = {"HK$": "HKD", "HKD": "HKD", "US$": "USD", "U.S.$": "USD", "USD": "USD", "RMB": "CNY", "CNY": "CNY",
             "C$": "CAD", "CAD": "CAD", "€": "EUR", "EUR": "EUR", "£": "GBP", "GBP": "GBP",
...
def check_pair(en_text: str, zh_text: str) -> list[dict]:
    """Return partial findings (type, en_span, zh_span, explanation) for one aligned pair."""
    found: list[dict] = []

    en_dates = extract_dates_en(en_text)
    zh_dates = extract_dates_zh(zh_text)
    en_nums = extract_numbers_en(en_text, [d.span for d in en_dates])
    zh_nums = extract_numbers_zh(zh_text, [d.span for d in zh_dates])
    en_cur = extract_currency(en_text, "en")
    zh_cur = extract_currency(zh_text, "zh")
    ...
    while unmatched_en or unmatched_zh:
        if unmatched_en and unmatched_zh:
            ...
            found.append({
                ...
                "en_span": list(a.span), "zh_span": list(b.span),
                "explanation": f"English states {a_txt}; Chinese states {b_txt}.",
            })
        elif unmatched_en:
            a = unmatched_en.pop(0)
            found.append({
                ...
                "explanation": f"English states {a.raw.strip()}; Chinese has no corresponding figure.",
            })
        else:
            ...
            found.append({
                ...
                "explanation": f"Chinese states {b.raw.strip()}; English has no corresponding figure.",
            })

    # ---- dates ------------------------------------------------------------
    label = "long stop date" if re.search(r"long\s+stop\s+date", en_text, re.I) else "date"
    ...
    while un_en or un_zh:
        if un_en and un_zh:
            ...
            found.append({
                "type": "DATE_MISMATCH",
                "en_span": list(a.span), "zh_span": list(b.span),
                "explanation": f"English {label} is {_fmt_date(a.iso)}; Chinese is {_fmt_date(b.iso)}.",
            })
        elif un_en:
            a = un_en.pop(0)
            found.append({
                "type": "DATE_MISMATCH", "en_span": list(a.span), "zh_span": None,
                "explanation": f"English states {_fmt_date(a.iso)}; Chinese has no corresponding date.",
            })
        else:
            b = un_zh.pop(0)
            found.append({
                "type": "DATE_MISMATCH", "en_span": None, "zh_span": list(b.span),
                "explanation": f"Chinese states {_fmt_date(b.iso)}; English has no corresponding date.",
            })
    ...
    if en_codes and zh_codes and en_codes != zh_codes:
        ...
        found.append({
            ...
            "explanation": (f"English uses {', '.join(sorted(en_codes))}; "
                            f"Chinese uses {', '.join(sorted(zh_codes))}."),
        })
    return found
    ...
    for sec in alignment.get("sections", []) or []:
        ...
        for pair in sec.get("pairs", []) or []:
            ...
            if not en_text or not zh_text:
                continue
            for f in check_pair(en_text, zh_text):
                findings.append({
                    "type": f["type"],
